...
final class RealXAIAPITests: XCTestCase {
  ...
  func testRealXAIAPIDataParsing() throws {
    ...
    let expectedInvoice = String(
      format: "Invoice limit $%.2f; used $%.2f; remain $%.2f", limitD, usedD, balanceD)
    ...
    let expectedPrepaid = String(
      format: "Prepaid credit $%.2f; used $%.2f; balance $%.2f", preCredD, preUsedD, preBalanceD)
